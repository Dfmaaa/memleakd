...
void *malloc(size_t size){
    ...
    insert_m(__ml_internal_store__ptrsh,ret,size);
    return ret;
}
void *calloc(size_t nmemb, size_t size){
    ...
    insert_m(__ml_internal_store__ptrsh,ret,nmemb*size);
    return ret;
}
void free(void *ptr){
    if(init==0){
    ...
    delete_vp(__ml_internal_store__ptrsh,ptr);
}
void *realloc(void *ptr, size_t size){
    ...
    if((n=find_m(*__ml_internal_store__ptrsh,ptr))==NULL){
        if(PERR_F){
